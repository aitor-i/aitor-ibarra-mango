...
function getRangeBoundary(rangeValues: RangeSliderProps): { max: number, min: number } {

  if (rangeValues.fixedValues && (rangeValues.min || rangeValues.max)) throw "Invalid props, you can't set fixed values and max or min values!";

  if (rangeValues.fixedValues) {
    const max = Math.max(...rangeValues.fixedValues);
    const min = Math.min(...rangeValues.fixedValues);

    return { max, min }

  }
  if (rangeValues.min !== undefined && rangeValues.max !== undefined) {
    return { max: rangeValues.max, min: rangeValues.min }
  }
  else {
    throw 'No max and min or fixed values found!'
  }
}
...
export const Range = ({ min, max, fixedValues }: RangeSliderProps) => {
  ...
  const rangeBoundary = getRangeBoundary({ min, max, fixedValues })

  useEffect(() => {
    setRange({ start: rangeBoundary.min ?? 0, end: rangeBoundary.max ?? 0 })
  }, []);
